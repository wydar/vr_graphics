...
THREEx.JsArucoMarker = function(){
	...
	var corner_prev_x = [0,0,0,0];
	var corner_prev_y = [0,0,0,0];
	var corner_diff_x = [0,0,0,0];
	var corner_diff_y = [0,0,0,0];
	var corner_max_x = [0,0,0,0];
	var corner_max_y = [0,0,0,0];
	...
	debugInfoElement.innerHTML = ''
		...
		+ '<div>Corner0: <span id="corner0">n/a</span></div>'
		+ '<div>Corner1: <span id="corner1">n/a</span></div>'
		+ '<div>Corner2: <span id="corner2">n/a</span></div>'
		+ '<div>Corner3: <span id="corner3">n/a</span></div>'
		+ '<div>Valid1: <span id="valid1">n/a</span></div>'
		+ '<div>Valid2: <span id="valid2">n/a</span></div>';
	 ...
	 */
	this.markerToObject3D = function(marker, object3d, long_focal){
		...
		posit = new POS.Posit(this.modelSize, long_focal);
		if (marker.id == controls.cualdebug) debugoutput = true;  // Visualizamos output de depuración para ese marcador
		var pose = posit.pose(corners, array_PersistIDs[marker.id], debugoutput); // Pasamos como referencia el objeto Persist del marcador
	...
	*/
	function drawDebug(markers, canvasElement){
		...
		for (var i = 0; i < markers.length; ++ i){
			...
			if (marker.id == controls.cualdebug){
				for (var k = 0; k<4; k++){
				$('#corner'+k.toString()).text("no"); // Borra valor de iteración previa
				}
			}
			...
			for (var j = 0; j < corners.length; ++ j){
				...
				corner_diff_x[j] = corner.x - corner_prev_x[j];
				corner_prev_x[j] = corner.x;
				corner_diff_y[j] = corner.y - corner_prev_y[j];
				corner_prev_y[j] = corner.y;
				
				corner = corners[(j + 1) % corners.length];
				context.lineTo(corner.x, corner.y);
				
				if (marker.id == controls.cualdebug){
					var stringdiff = corner_diff_x[j].toString();
					if (corner_diff_x[j] > 3) stringdiff += "Xlarge";
					stringdiff += ", " + corner_diff_y[j].toString();
					if (corner_diff_y[j] > 3) stringdiff += "Ylarge";
					$('#corner'+j.toString()).text(stringdiff);  // Debug output diff en corner1
				}				
			}
